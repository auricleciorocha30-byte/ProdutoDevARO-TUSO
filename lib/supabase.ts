...
let TURSO_URL = localStorage.getItem('master_db_url') || 'https://produtodevaro-auricleciorocha30-byte.aws-us-east-1.turso.io';
let TURSO_AUTH_TOKEN = localStorage.getItem('master_db_token') || 'eyJhbGciOiJFZERTQSIsInR5cCI6IkpXVCJ9.eyJhIjoicnciLCJpYXQiOjE3NzIxMzYwOTMsImlkIjoiMDE5YzliNzctZGMwMS03MmIwLWFmYWItYWRlOGY0MjM5YTBjIiwicmlkIjoiYTQyYjFmY2EtYjc0YS00MGMwLTk3M2QtODlmNmFlMTBkYzFiIn0.A7LAbG4yZ70-XPczvHXgaVUm2t_rJuTlsMpefd86FVprMb50rPZU5aICZdVvQvXpdnwOiav_nNMRCOOmi2cQDQ';

export const setMasterDbConfig = (url: string, token: string) => {
    if (url) localStorage.setItem('master_db_url', url);
    else localStorage.removeItem('master_db_url');
    
    if (token) localStorage.setItem('master_db_token', token);
    else localStorage.removeItem('master_db_token');
    
    TURSO_URL = url || 'https://produtodevaro-auricleciorocha30-byte.aws-us-east-1.turso.io';
    TURSO_AUTH_TOKEN = token || 'eyJhbGciOiJFZERTQSIsInR5cCI6IkpXVCJ9.eyJhIjoicnciLCJpYXQiOjE3NzIxMzYwOTMsImlkIjoiMDE5YzliNzctZGMwMS03MmIwLWFmYWItYWRlOGY0MjM5YTBjIiwicmlkIjoiYTQyYjFmY2EtYjc0YS00MGMwLTk3M2QtODlmNmFlMTBkYzFiIn0.A7LAbG4yZ70-XPczvHXgaVUm2t_rJuTlsMpefd86FVprMb50rPZU5aICZdVvQvXpdnwOiav_nNMRCOOmi2cQDQ';
};
